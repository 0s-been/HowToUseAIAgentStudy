...
## 11단계 — 프레임당 이동/회전량 상한 (hitch, 빠른 마우스 동작 대응)

### 배경
10단계에서 바닥 격자의 공간적 앨리어싱(정지 상태 또는 느린 이동에서 칸이 어른거리는
현상)은 해결했지만, `Shift`로 빠르게 이동하거나 마우스를 아주 빠르게 돌릴 때는
여전히 화면이 번지는 느낌이 있었다. 이번에는 격자뿐 아니라 큐브에서도 나타났다 —
큐브는 체커 필터링과 전혀 무관한 오브젝트라, 원인이 10단계와는 다른 곳에 있다는
확실한 단서였다.

### 진짜 원인: "프레임 갱신"이 아니라 "프레임당 이동/회전량"이 무제한이었다
프레임 동기화(펜스 대기, 프레임별 자원 분리, MSAA 타겟의 단일 커맨드 큐 직렬 실행
보장)를 다시 감사했지만 결함은 없었다. 대신 전혀 다른 곳에 실제 문제가 있었다 —
**`Timer`의 `deltaTime`에도, `InputReader`의 마우스 픽셀 델타에도 상한이 없었다.**

- **`Shift` + hitch(짧은 멈춤)**: 디버거 일시정지·알트탭·드라이버 지연 등으로 한
  프레임이 오래 걸리면 다음 `Timer::Tick()`의 델타가 순간적으로 커진다. 이동
  거리는 `speed * deltaTime`이라, `Shift`로 속도를 3배 올린 상태에서 그 큰 델타를
  그대로 곱하면 카메라가 한 프레임 만에 크게 점프한다.
- **아주 빠른 마우스 동작**: 손목을 빨리 스냅하면 한 프레임에 누적되는 원시 마우스
  픽셀 이동량이 수백~수천에 달할 수 있다. 감도(0.25도/픽셀)를 곱해도 프레임당
  회전량이 수백 도에 이를 수 있다.

두 경우 모두 결과는 같다 — 연속된 두 프레임이 서로 거의 무관한 장면을 보여주고,
그 큰 차이를 사람 눈과 디스플레이는 매끄러운 이동이 아니라 "번짐"으로 인지한다.
MSAA와 체커 필터링은 **한 프레임 안**의 공간적 앨리어싱을 다루는 기법이라, 이런
**프레임 사이**의 과도한 점프에는 애초에 관여하지 않는다 — 그래서 격자뿐 아니라
필터링과 무관한 큐브에서도 똑같이 나타난 것이다.

### 수치로 확인
```
마우스 원시 델타 400px/프레임 -> 회전량: 상한 전 100.0도 / 상한 후 45.0도
마우스 원시 델타 800px/프레임 -> 회전량: 상한 전 200.0도 / 상한 후 45.0도
프레임 간격 300ms(hitch)  -> Shift 이동 거리: 상한 전 4.5단위 / 상한 후 1.5단위
```
실제로 흔히 있을 수 있는 빠른 마우스 동작만으로도 한 프레임에 100~200도씩 시야가
돌아갈 수 있었다는 뜻이다 — 정상적인 회전이 아니라 사실상 다른 장면으로 순간이동하는
수준이라, 어떤 안티앨리어싱으로도 매끄러워 보이게 만들 수 없다.

### 추가·수정한 파일
| 파일 | 내용 |
|---|---|
| `Core/Timer.h/.cpp` | `kMaxDeltaTime`(0.1초) 도입. 시뮬레이션용 델타에만 적용하고, FPS 통계는 원본 델타를 그대로 사용 |
| `Core/Application.h/.cpp` | `m_maxRotationPerFrame`(45도) 도입. 마우스 yaw/pitch 회전량을 감도 적용 후 한 번 더 clamp |

### 결정 사항과 이유

**FPS 통계는 clamp 되지 않은 원래 델타로 계산한다.** `Timer::UpdateFrameStats`에
`rawDeltaTime`을 그대로 넘긴다. 그래야 타이틀바의 fps가 진짜 성능을 보여주고,
이번처럼 "정말 프레임이 느려진 건지" 사용자가 직접 확인할 수 있는 진단 수단으로
남는다. 반면 `GetDeltaTime()`이 돌려주는, 실제 이동/회전 계산에 쓰이는 값에만
상한을 씌운다.

**상한값은 넉넉하게 잡았다.** `kMaxDeltaTime = 0.1`초(초당 10프레임 아래로 떨어진
경우만 해당)와 `m_maxRotationPerFrame = 45`도(보통의 빠른 시선 이동은 훨씬 못
미친다)는 정상적인 빠른 조작은 전혀 방해하지 않으면서, 병적으로 큰 단일 프레임
점프만 막도록 골랐다.

**InputReader가 아니라 Application에 회전 clamp를 둔다.** `InputReader`는 "지금 이
가상 키가 눌려 있는가", "마우스가 몇 픽셀 움직였는가" 같은 원시 사실만 보고하는
계층으로 남겨 뒀다(6단계 설계 원칙). "카메라 회전은 프레임당 몇 도를 넘지 않는다"는
카메라/애플리케이션의 정책이지 입력 장치의 사실이 아니므로, 그 정책이 실제로
쓰이는 자리인 `Application::UpdateCamera`에 둔다.

### 이 단계의 확인 방법
마우스를 최대한 빠르게 휙 돌려 봐도 시야가 한 프레임 만에 완전히 다른 방향으로
뒤집히지 않고, 빠르지만 따라갈 수 있는 정도로 회전이 제한된다. 창을 잠깐 드래그해
강제로 멈췄다 놓아도(hitch를 인위적으로 재현) `Shift` 이동 중이던 큐브/카메라가
순간이동하듯 튀지 않는다.

---

## 12단계 — "잔상" 재조사: 앱 문제가 아니라 디스플레이 특성으로 결론

### 배경
11단계의 clamp를 적용한 뒤에도 Shift로 빠르게 이동하거나 마우스를 빠르게 돌리면
여전히 또렷한 유령 이미지가 겹쳐 보이는 "잔상"이 남는다는 보고가 있었다.
프레임 동기화(펜스, MSAA 타겟의 단일 큐 직렬 실행)를 다시 감사했지만 결함이
없었고, 실측 결과 fps는 vsync on/off와 무관하게 항상 60으로 고정되어 있어
프레임 페이싱 문제도 아니었다.

### 결정적 테스트
앱 화면을 정지된 스크린샷으로 저장한 뒤, **그 스크린샷 이미지를 화면에서
움직였을 때도 동일한 잔상이 나타났다.** 스크린샷은 완전히 정적인 이미지라
DX12 렌더링 코드(펜스, MSAA, 셰이더 등)가 전혀 관여하지 않는다. 그런데도 같은
현상이 재현됐다는 것은, 원인이 앱이 아니라 **모니터의 표시 방식(sample-and-hold
응답 특성)과 움직이는 물체를 눈이 따라가며 인지하는 방식**에 있다는 뜻이다.
코드로 고칠 수 있는 영역이 아니다.

### 결론
이 저장소의 DX12Framework 코드에는 이 현상과 관련된 결함이 없다. 정지 화면을
옮기기만 해도 재현되므로 렌더러의 프레임 생성/동기화와 무관함이 확인됐다.
11단계에서 넣은 `Timer::kMaxDeltaTime`, `Application::m_maxRotationPerFrame`
clamp는 이 현상을 고치지는 못했지만 hitch·과도한 입력에 대한 안전장치로는
여전히 유효해 그대로 두었다.

실제 렌더러 안에서 이런 종류의 체감을 줄이려면 모션 블러(오브젝트별 이전
프레임 변환을 기록해 노출 시간 동안의 궤적을 흐리는 후처리 기법)를 구현하는
방법이 있지만, 지금은 진행하지 않기로 했다. 새로 손댈 일이 생기면 그때
확장하는 것으로 남겨 둔다.
...
토글할 수 있다. hitch나 아주 빠른 마우스 동작으로 한 프레임 만에 화면이 크게 튀는
것도 막아 뒀다(11단계).
...
6. **모션 블러** (선택) — 12단계에서 확인한 "잔상"은 앱 버그가 아니라 모니터
   표시 방식(sample-and-hold)에서 오는 현상이라 급한 항목은 아니다. 체감을
   줄이고 싶어지면 오브젝트별 이전 프레임 변환(속도 버퍼)을 기록하고 후처리
   블러 패스를 추가하는 방향으로 확장한다.
